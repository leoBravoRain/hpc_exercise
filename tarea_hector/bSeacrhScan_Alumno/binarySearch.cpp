...
void genArrays(ParProg *par){
	...
	uint extra = par->nc*(sizeof(uint)+sizeof(ulong));
	cout << " Extra size for COUNT[] + POS[] = " << extra/1024.0 << " KiB" << endl;
...
bool binarySearch(ParProg *par, ulong x, ulong *idx){
	...
	while (l<=r){
		...
		//  get middle
		// m = l + (r-l)/2;
		m = ( r + l) >> 1; // Bitwise operation

		// First is the most probably option
		if (x<A[m])
