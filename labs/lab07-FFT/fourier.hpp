...
{   
    // std::fstream in("signal_2_20.dat"); //  
    // std::fstream in("signal_2_18.dat"); //  
    // std::fstream in("signal_2_17.dat"); //  
    std::fstream in("signal_2_15.dat"); // 
    // std::fstream in("signal_2_10.dat"); // 
    // std::fstream in("signal_2_5.dat");
...
// FFT Recursive
void fft_recursive_par(TYPE* x, TYPE* X_real, TYPE* X_imag, int N, int N_orig, int par_number)
{
    // N max: 14
    // printf("N: %i \n", N);
    ...
    else
    {
        ...
        if(mylog2(N_orig/N) == par_number){
...

            {
                ...
                // printf("parallel section with threads: %i\n", omp_get_num_threads());

                // Assign one task to thread 0
                if(tid == 0) {

                    // printf("'thread %i'\n", tid);
                    #pragma omp single
                    fft_recursive_par(x_even, X_real_even, X_imag_even, N/2, N, par_number);

                }

                // assign othera task to thread 1
                else if(tid == 1){

                    // printf("'thread %i'\n", tid);
                    #pragma omp single
                    fft_recursive_par(x_odd, X_real_odd, X_imag_odd, N/2, N, par_number);       
                }
        ...
        // if the level is not the desire, do sequential recursive
        else{

            // printf("threads: %i;    secuencial. N = %i;    N_or/N = %i \n", omp_get_num_threads(), N, mylog2(N_orig/N));

            fft_recursive_par(x_even, X_real_even, X_imag_even, N/2, N_orig, par_number);
            fft_recursive_par(x_odd, X_real_odd, X_imag_odd, N/2, N_orig, par_number);       
